...
# ---------- 配置 ----------
LOCAL_MODEL_PATH = r".\exported_merged_model"
...
MYSQL_USER, MYSQL_PASSWORD = "root", os.getenv("MYSQL_PASSWORD", "mysql密码")
...
class ChatRequest(BaseModel):
    ...
    history_len: int = 5           # 使用最近多少轮对话（一轮 = 一条user + 一条assistant）
    # history 字段将被忽略，不再使用前端传入的历史
    history: List[Dict[str, str]] = []   # 保留但不再使用
...
async def save_message(conv_id: str, role: str, content: str = None, tool_calls: List[Dict] = None,
    ...
    await database.execute(
        "INSERT INTO agentmessages (conversation_id, role, content, tool_calls, tool_call_id) "
...
async def load_history_from_db(conv_id: str, history_len: int) -> List[Dict[str, str]]:
    """
    从数据库加载指定会话的历史消息（只含 user 和 assistant 角色），
    按时间升序返回最近 history_len 轮对话（一轮 = 一条 user 和一条 assistant）。
    如果 history_len <= 0 返回空列表。
    """
    if history_len <= 0:
        return []
    # 查询该会话所有 user 和 assistant 消息，按时间升序
    rows = await database.fetch_all(
        "SELECT role, content FROM agentmessages WHERE conversation_id = :cid AND role IN ('user', 'assistant') "
        "ORDER BY created_at ASC",
        {"cid": conv_id}
    )
    # 转换为字典列表
    messages = [{"role": row["role"], "content": row["content"]} for row in rows]
    # 取最后 history_len * 2 条消息（每条 user+assistant 为两消息）
...
def clean_output(text: str) -> str:
    ...
    # 移除末尾可能出现的 'fq', 'ankind', '怎fq' 等异常
    text = re.sub(r'[\u4e00-\u9fa5a-zA-Z]{0,5}[a-zA-Z]{2,}$', '', text)
    text = text.replace('�', '')
    text = re.sub(r'[^\u4e00-\u9fa5a-zA-Z0-9，。！？；：""''（）、《》【】\s]+$', '', text)
    return text.strip()

# ---------- 核心对话（从数据库读取历史，忽略前端传入的 history）----------
async def complete_with_tools(req: ChatRequest, conv_id: str):
    ...
    # 从数据库加载历史消息（忽略前端传入的 req.history）
    history_messages = await load_history_from_db(conv_id, req.history_len)
    ...
    for iteration in range(5):
        ...
        final_content = clean_output(assistant_content)
...
if __name__ == "__main__":
    import uvicorn
    print("=" * 60)
    print("🏥 医疗AI助手后端服务 (基于数据库历史，不依赖前端传输)")
    print("=" * 60)
